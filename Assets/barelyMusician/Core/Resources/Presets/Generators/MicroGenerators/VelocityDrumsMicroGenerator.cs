...
namespace BarelyAPI
{
    public class VelocityDrumsMicroGenerator : MicroGenerator
    {

        public VelocityDrumsMicroGenerator(Sequencer sequencer)
            : base(sequencer)
        {
        }
        ...
        protected override void generateLine(SectionType section, int bar, int harmonic, ref List<NoteMeta> line)
        {
            ...
            {
                float offset = 0.0f;
                float duration = 0.25f / LineLength;
                ...
                for (int i = 0; i < LineLength * 4; ++i)
                {
                    ...
                    // hihat open
                    if (RandomNumber.NextFloat(0.0f, 1.0f) < 0.025f)
