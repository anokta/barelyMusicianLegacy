...
namespace BarelyAPI
{
    public class ZChordMicroGenerator : MicroGenerator
    {
        int[] chord = { 0, 2, 4, 6, 7 };

        public ZChordMicroGenerator(Sequencer sequencer)
            : base(sequencer)
        {
        }
            ...
            switch (section)
            {
                ...
                case SectionType.BRIDGE:
                    for (int i = 0; i < 3; ++i)
                    {
                        ...
                        {
                            line.Add(new NoteMeta(harmonic + chord[i], 0.25f * j, 0.15f, 0.9f));
                        }
                    }
                    break;
            }
